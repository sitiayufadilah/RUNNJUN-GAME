...
let sonic,
  sonicAction = "run",
  sonicHasCrashed = false,
  currentLane = 0,
  maxHeight = 0.5,
  dRot = 45,
  strafeDuration = 0.5,
  actionClock = { startTime: undefined, duration: 667 },
  animations = { run: undefined, roll: undefined, jump: undefined, fall: undefined, dead: undefined };
let obstacles = [],
  coins = [],
  coinSize = 0.025,
  rows = 9,
...
class Cloud {
  constructor(size = 0.2, cols = 12, rows = 12) {
    ...
    this.obj = new THREE.Mesh(
      THREE.BufferGeometryUtils.mergeBufferGeometries([sphere1Geometry, sphere2Geometry, sphere3Geometry, sphere4Geometry, sphere5Geometry, sphere6Geometry, sphere7Geometry, sphere8Geometry, sphere9Geometry]),
      material
    );
    this.obj.scale.set(size, size, size);
    return this.obj;
...
const startGame = () => {
  swal({
    ...
    text: /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)
      ? "Swipe up to jump, down to roll, and left/right to switch lanes."
...
const strafe = (strafeLeft) => {
  ...
  let rotation = new THREE.QuaternionKeyframeTrack(
    ".quaternion",
    [0, strafeDuration / 2, strafeDuration],
...
const readdToPath = () => {
  obstacles.forEach((obstacle) => {
    if (obstacle.position.z > sonic.position.z + gap) {
      //gone beyond field of view
      ...
      switch (obstacle.userData.type) {
        case "RoadBlock":
          crashed =
            sonic.position.x > obstacle.position.x - laneWidth / 2 &&
            sonic.position.x < obstacle.position.x + laneWidth / 2 &&
            obstacle.position.z + 0.5 * Math.tan(THREE.Math.degToRad(21)) > sonic.position.z - 0.1 &&
            obstacle.position.z - 0.5 * Math.tan(THREE.Math.degToRad(21)) < sonic.position.z + 0.1;
          break;
        case "SmallRoadBlock":
          crashed =
            sonic.position.x > obstacle.position.x - laneWidth / 2 &&
            sonic.position.x < obstacle.position.x + laneWidth / 2 &&
            obstacle.position.z + 0.0375 > sonic.position.z - 0.1 &&
            obstacle.position.z - 0.0125 < sonic.position.z + 0.1 &&
            sonicAction != "roll";
          break;
        case "Cone":
          crashed =
            sonic.position.x > obstacle.position.x - laneWidth / 2 &&
            sonic.position.x < obstacle.position.x + laneWidth / 2 &&
            obstacle.position.z + 0.125 > sonic.position.z - 0.1 &&
            obstacle.position.z - 0.125 < sonic.position.z + 0.1 &&
            sonicAction != "jump";
          break;
        case "Spike":
          crashed =
            sonic.position.x > obstacle.position.x - laneWidth / 2 &&
            sonic.position.x < obstacle.position.x + laneWidth / 2 &&
            obstacle.position.z + 0.05 > sonic.position.z - 0.1 &&
            obstacle.position.z - 0.05 < sonic.position.z + 0.1 &&
            sonicAction != "jump";
          break;
      }
          ...
          if (sonic.position.x > obstacle.position.x - laneWidth / 2 && sonic.position.x < obstacle.position.x + laneWidth / 2 && obstacle.position.z + coin.position.z >= sonic.position.z) {
            ...
            if (coin.visible) {
              ...
              if (score % 100 == 0 && score != 0) {
                ...
                let index = 1,
                  colors = ["#ff0000", "#ffa500", "#ffff00", "#00ff00", "#0000ff", "#ee82ee"],
